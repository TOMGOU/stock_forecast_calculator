...
function calStockPrice() {
  ...
  const growth1 = +stockInfo.growth1
  const growth2 = +stockInfo.growth2
  const growth3 = +stockInfo.growth3
  const growth4 = +stockInfo.growth4
  ...
  return [
    ...
    {years: '净利增长率', last_year: '--', this_year: `${(growth1 * 100).toFixed(0)}%`, next_year: `${(growth2 * 100).toFixed(0)}%`, after_year: `${(growth3 * 100).toFixed(0)}%`, future_year: `${(growth4 * 100).toFixed(0)}%`}
  ]
}
